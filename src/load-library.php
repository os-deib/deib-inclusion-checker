...
/**
 * This function checks if WordPress needs to request
 * the language. If the language is not loaded yet,
 * the request functions will be triggered.
 *
...
function deibic_maybe_load_library() {
	...
	# check if we are on the block editor because we only
	# need to load the library there
	if ( true !== $current_screen->is_block_editor ) {
		return;
	}

	# get the locale of the current blog to determine
	# if the setted language is already refined
	$locale = get_locale();
	if ( ! in_array( $locale, deibic_get_refined_languages() ) ) {
		return;
	}
	...
	# fetch the language if there hasen't been a fetch yet
	if ( ! in_array( $locale, array_keys( $last_fetches ) ) ) {
